...
//        $userID = $this->loginUser;
        ...
        $items=$objNotification->SessionNotificationCount($userID[0]->user_id);        
        ...
        $companyId = Company::select('id')->where('user_id', $userID[0]->user_id)->first();
        ...
        $data['js'] = array('employee/companyticket.js');
        ...
        return view('employee.companyticket.ticket-list', $data);
    }
...
//        $companyId = Company::select('id')->where('user_id', $userid)->first();
        $employee_list = $objEmployee->getEmployeeList($company_Id);
        ...
        $data['pluginjs'] = array('jQuery/jquery.validate.min.js');
        $data['js'] = array('employee/companyticket.js', 'jquery.form.min.js');
        ...
        return view('employee.companyticket.ticket-add', $data);
    }
    ...
    public function ajaxAction(Request $request) 
    {
        ...
        switch ($action) {
            case 'getdatatable':
                $objTicket = new Ticket();
                $ticketList = $objTicket->getdatatableCompany($request);
    ...
    public function viewTicketComments($id,Request $request){
        ...
        if ($request->isMethod('post')) {
            ...
            $result = $objTicketComment->saveTicketCommentCompany($request);
            if($result) {
                $return['status'] = 'success';
        ...
        return view('employee.companyticket.ticket-comments', $data);
    }
