                        ...
                        @if($cmpMails)
                            @foreach($cmpMails as $emailList)
                                <tr class="unread">
                                    ...
                                    @if($emailList['file'])
                                        <td class=""><a href="{{ url('/company/send-mail-detail/'.$emailList['id']) }}"><i class="fa fa-paperclip"></i></a></td>
                                    @else
                                        <td class=""></td>
                                    @endif
                                    <td>
                                        <a href="{{ url('/company/send-mail-detail/'.$emailList['id']) }}">{{ strlen(strip_tags($emailList['message'])) > 18 ? substr(strip_tags($emailList['message']),0,18)."..." : strip_tags($emailList['message']) }}</a>
                                    </td>
                                    <td class="text-right mail-date">
                                         <a href="{{ url('/company/send-mail-detail/'.$emailList['id']) }}">{{ date('Y-m-d H:i A', strtotime($emailList['created_at'])) }}</a>
                                    </td>
                                </tr>
                            @endforeach
                        @else
